...
func (e *Entry) StrFmt(key string, format string, v ...any) *Entry {
	if e == nil {
		return nil
	}
...
// HexOptions defines options for hex encoding
type HexOptions struct {
	Separator byte // separator between hex bytes (default: 0, no separator)
	Length    int  // maximum length of bytes to encode (default: all)
	Lowercase bool // use lowercase hex characters (default: false, uppercase)
}

// Hex adds the field key with val as a hex string to the entry.
// Optional parameters can be provided to customize the hex encoding.
func (e *Entry) Hex(key string, val []byte, opts ...HexOptions) *Entry {
	if e == nil {
		return nil
	}
	...
	if len(opts) > 0 {
		...
		if opt.Length > 0 && opt.Length < length {
			length = opt.Length
		}
		lowercase = opt.Lowercase
	}

	// Choose hex character set (default uppercase)
	...
	for i := 0; i < length; i++ {
		v := val[i]
		if i > 0 && separator != 0 {
			e.buf = append(e.buf, separator, hexChars[v>>4], hexChars[v&0x0f])
		} else {
			e.buf = append(e.buf, hexChars[v>>4], hexChars[v&0x0f])
		}
	}
